...
const TABS = [
  ...
  { id: "importar", label: "Importar", icon: "M12 3v12m0 0 4-4m-4 4-4-4M4 17v2a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-2" },
...
export default function BottomNav({ tab, setTab, session }) {
  const visible = TABS.filter((t) => canAccess(session, t.id));
  return (
    <nav aria-label="Navegación principal" className="fixed inset-x-0 bottom-0 z-40 pb-[env(safe-area-inset-bottom)]">
      <div className="glass mx-auto mb-3 flex w-[min(32rem,calc(100%-1.5rem))] justify-around !rounded-2xl p-1.5">
        {visible.map((t) => {
          ...
          return (
            <button
              ...
              className="pressable relative flex min-w-13 flex-col items-center gap-0.5 rounded-xl px-2 py-1.5 text-[11px] font-medium"
            >
              {active && (
                <motion.span
                  layoutId="nav-pill"
