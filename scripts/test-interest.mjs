...
const baseAcc = (over) => ({
  ...
  rate: 0, accrual: "none", lastAccrual: "2026-06-16", isrRate: 0.000524, ...over,
});
const run = (acc, now) => {
...
console.log("== 2. SOFIPO MXN capped → SOLO interés, SIN ISR ==");
{
  const st = run(baseAcc({
    type: "sofipo", capped: true, balance: 25000, isrRate: 0,
  ...
  ok(find(st, "Impuesto").length === 0, "SOFIPO NO debe tener ISR (isrRate=0)");
}
...
console.log("== 3. Inversión MXN capped → interés + ISR ==");
{
  ...
  ok(find(st, "Impuesto").length === 1, "inversión capped DEBE tener ISR (isrRate=0.000524 del baseAcc)");
}
...
console.log("== 6. Cuentas sin isrRate → sin ISR ==");
{
  const ahorro = run(baseAcc({ type: "savings", rate: 0.04, accrual: "daily", isrRate: 0 }), "2026-06-17");
  ok(find(ahorro, "Impuesto").length === 0, "ahorro con isrRate=0 no paga ISR");
  const usd = run(baseAcc({ type: "investment", currency: "USD", rate: 0.05, accrual: "daily", isrRate: 0 }), "2026-06-17");
  ok(find(usd, "Impuesto").length === 0, "inversión USD con isrRate=0 no paga ISR");
}

console.log("== 7. SOFIPO capped 2 tramos → 2 intereses, 0 ISR ==");
{
  const st = run(baseAcc({
    type: "sofipo", capped: true, balance: 50000, isrRate: 0,
  ...
  ok(find(st, "Impuesto").length === 0, "SOFIPO 2 tramos sin ISR (isrRate=0)");
}
